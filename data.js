...
(() => {
  ...
  const navGroups = [
    ...
    {title:"Glomerular diseases",items:[
      {id:"gn-overview",label:"GN concepts & mechanisms"},{id:"gn-patterns",label:"Clinical syndromes"},{id:"gn-workflow",label:"Diagnostic workflow"},{id:"gn-biopsy",label:"Biopsy pattern recognition"},{id:"gn-management",label:"Supportive & immune care"},{id:"gn-mcd-fsgs",label:"MCD & FSGS"},{id:"gn-membranous",label:"Membranous nephropathy"},{id:"gn-infection",label:"Infection-related GN"},{id:"gn-rpgn",label:"Rapidly progressive GN"},{id:"gn-anca-antigbm",label:"ANCA & anti-GBM"},{id:"gn-mpgn-c3g",label:"MPGN & C3G"},{id:"gn-iga",label:"IgA nephropathy"},{id:"gn-col4",label:"COL4 / Alport spectrum"},{id:"gn-amyloid",label:"Renal amyloidosis"},{id:"gn-summary",label:"High-yield correction matrix"}
    ]},
    {title:"Interactive revision",items:[
      {id:"nephron-lab",label:"Nephron transport lab"},{id:"filtration-lab",label:"Filtration forces lab"},{id:"crcl-lab",label:"Creatinine clearance lab"},{id:"urine-interpreter",label:"Urine pattern interpreter"},{id:"proteinuria-lab",label:"Proteinuria classifier"},{id:"gn-pattern-lab",label:"GN pattern interpreter"},{id:"clinical-cases",label:"Clinical cases"},{id:"flashcards",label:"Flashcards"},{id:"quiz",label:"Scored quiz"},{id:"sources",label:"Sources & original PDFs"}
    ]}
  ];
  ...
  const clusters = {
    ...
    glomerular:["gn-overview","gn-patterns","gn-workflow","gn-biopsy","gn-management","gn-mcd-fsgs","gn-membranous","gn-infection","gn-rpgn","gn-anca-antigbm","gn-mpgn-c3g","gn-iga","gn-col4","gn-amyloid","gn-summary"],
    revision:["nephron-lab","filtration-lab","crcl-lab","urine-interpreter","proteinuria-lab","gn-pattern-lab","clinical-cases","flashcards","quiz","sources"]
  };

  const modules = {
    overview:M("Renal Medicine Lab","Five connected chapters · one clinical model","Move from normal renal structure and physiology to question-driven investigations, then apply the same logic to proteinuria, hematuria, and glomerular disease. Every module links forward to interpretation, cases, flashcards, and scored revision.",["53 connected modules","5 original PDFs","Offline-first"],()=>`
      ${section("The whole renal map",`${kidneyDiagram()}${callout("The central model","The glomerulus filters, the tubule modifies, and the collecting duct finalizes urine. Clinical investigation asks: Is filtration reduced? Is kidney damage present? Is homeostasis failing? Is the source glomerular, tubular, structural, vascular, or urologic? Will biopsy change treatment?","success")}`)}
      ${section("Five layers of understanding",cards([
        ...
        {icon:"2",title:"Investigations",body:"<p>Interpret trends in creatinine and eGFR together with urinalysis, microscopy, targeted imaging, serology, and selective biopsy.</p>"},
        ...
        {icon:"5",title:"Glomerular disease",body:"<p>Classify nephritic, nephrotic, mixed, and rapidly progressive syndromes by immune pattern, complement, biopsy, and disease-specific management.</p>"}
      ]))}
      ${section("A reusable bedside algorithm",flow([
        {title:"Stabilize",body:"Hyperkalemia, pulmonary edema, acidosis, sepsis, anuria, clot retention, rapidly progressive kidney failure, or pulmonary hemorrhage come first."},
        {title:"Confirm",body:"Check prior results, collection quality, microscopy, urine output, drugs, infection, and transient triggers."},
        {title:"Localize",body:"Combine kidney function, albumin/protein, sediment, symptoms, complement, serology, and imaging."},
        {title:"Classify",body:"Define nephritic, nephrotic, mixed, isolated urinary, or rapidly progressive presentation; then identify immune-complex, anti-GBM, pauci-immune, complement-dominant, podocyte, hereditary, or deposition disease."},
        {title:"Act",body:"Treat reversible causes, give supportive kidney protection, use immunosuppression only when disease-specific, and biopsy when tissue will alter diagnosis, prognosis, or therapy."}
      ]))}
      ${section("Start studying",`${stats([{value:"9",label:"principles pages"},{value:"13",label:"investigation pages"},{value:"12",label:"proteinuria pages"},{value:"11",label:"hematuria pages"},{value:"20",label:"glomerular pages"}])}${routeLinks([["renal-anatomy","Begin with anatomy"],["investigation-framework","Jump to investigations"],["gn-overview","Study glomerular disease"],["clinical-cases","Test clinical reasoning"]])}`)}
    `),
    ...
    "gn-overview":M("Glomerular Disease: Core Concepts","Chapter 5 · glomerular diseases","Glomerulonephritis is inflammatory injury of the filtration unit; glomerulopathy is broader and includes podocyte and deposition disorders. Start with the barrier, then classify the immune pattern.",["Barrier injury","Five pathogenic patterns","Crescents are a response"],()=>`
      ${section("How glomerular injury becomes visible",table(["Site or process","Consequence","Clinical clue"],[
        ["Capillary-wall disruption","Red cells cross the barrier","Dysmorphic RBCs, acanthocytes, RBC casts, smoky or cola urine."],
        ["Loss of size/charge selectivity","Albumin and other proteins enter urine","Albuminuria, subnephrotic or nephrotic-range proteinuria."],
        ["Inflammation and capillary obstruction","GFR falls while sodium is retained","Oliguria, azotemia, edema and hypertension."],
        ["Podocyte injury","Severe protein leak with little visible inflammation","Nephrotic syndrome and a bland or minimally active sediment."],
        ["Mesangial/endocapillary proliferation","Filtration surface falls and hematuria develops","Nephritic or mixed nephritic-nephrotic syndrome."],
        ["Chronic sclerosis and interstitial fibrosis","Irreversible nephron loss","Progressive CKD and small kidneys late in disease."]
      ]))}
      ${section("Five immune-pathogenic patterns",cards([
        {icon:"IC",title:"Immune-complex",body:"<p>Circulating or in-situ immune complexes activate complement. Examples: IgA nephropathy, infection-related GN, lupus. IF is usually granular.</p>"},
        {icon:"GBM",title:"Anti-GBM",body:"<p>Autoantibodies bind type IV collagen. IF shows strong linear IgG along the GBM.</p>"},
        {icon:"0",title:"Pauci-immune",body:"<p>ANCA-associated necrotizing small-vessel vasculitis with little or no immune staining.</p>"},
        {icon:"C3",title:"Complement-dominant",body:"<p>Alternative-pathway dysregulation produces C3-dominant staining, as in C3 glomerulopathy.</p>"},
        {icon:"P",title:"Podocytopathy",body:"<p>Primary podocyte dysfunction with little immune deposition, as in minimal change disease and primary FSGS.</p>"}
      ]))}
      ${callout("Crescent is not a diagnosis","A crescent is a severe histologic response to capillary-wall rupture. Anti-GBM disease, ANCA vasculitis, immune-complex GN and severe IgA nephropathy can all be crescentic.","warning")}
    `),

    "gn-patterns":M("Clinical Syndromes in Glomerular Disease","Chapter 5 · pattern recognition","Define the presentation before naming a disease. The same biopsy pattern can produce different syndromes, and the same syndrome can arise from several mechanisms.",["Nephritic","Nephrotic","Mixed","RPGN"],()=>`
      ${section("Syndrome map",table(["Clinical syndrome","Dominant findings","Common patterns"],[
        ["Asymptomatic urinary abnormality","Microscopic hematuria and/or subnephrotic proteinuria with preserved GFR","IgA nephropathy, COL4-related disease, early immune-complex GN."],
        ["Acute nephritic syndrome","Hematuria, RBC casts, variable proteinuria, edema, hypertension and reduced GFR","Infection-related GN, IgA nephropathy, lupus nephritis."],
        ["Nephrotic syndrome",">3.5 g/day proteinuria in adults plus hypoalbuminemia and edema","MCD, membranous nephropathy, primary FSGS, amyloidosis."],
        ["Mixed nephritic-nephrotic","Active sediment plus heavy proteinuria","MPGN pattern, lupus, infection-related GN, severe IgA nephropathy."],
        ["Rapidly progressive GN","Rapid loss of kidney function over days to weeks, active sediment and often crescents","ANCA vasculitis, anti-GBM disease, crescentic immune-complex GN."],
        ["Chronic GN / CKD","Persistent abnormalities with falling GFR and chronic scarring","Any unresolved progressive glomerular disease."]
      ]))}
      ${section("Primary, secondary and genetic categories",table(["Category","Examples","Practical implication"],[
        ["Primary renal disease","Primary MN, primary FSGS, MCD, primary IgA nephropathy","Treat according to kidney risk, pathology and response."],
        ["Infection-associated","Streptococcal or staphylococcal infection, endocarditis, HBV, HCV, HIV","Eradicate infection; immunosuppression may be harmful."],
        ["Systemic immune disease","SLE, IgA vasculitis, cryoglobulinemia, ANCA vasculitis","Treat systemic disease and renal activity together."],
        ["Monoclonal / malignant","MGRS, lymphoma, myeloma, AL amyloid","Urgent hematologic evaluation and clone-directed therapy."],
        ["Drug or toxin related","NSAIDs, hydralazine, anti-TNF therapy and others","Stop culprit and classify the actual lesion."],
        ["Genetic / hereditary","COL4A3/4/5 disease, Fabry disease, genetic FSGS","Genetics can change immunosuppression, family screening and transplant counseling."]
      ]))}
      ${callout("Terminology update","Diabetes, amyloidosis, MCD and FSGS are important glomerular diseases, but several are not inflammatory glomerulonephritides.","info")}
    `),

    "gn-workflow":M("Diagnostic Workflow for Suspected GN","Chapter 5 · diagnostic reasoning","Confirm glomerular injury, assess urgency, define the syndrome, search for a cause, order targeted serology and biopsy when it will change care.",["Urine microscopy","Complement + serology","Biopsy when decisive"],()=>`
      ${section("Six-step sequence",flow([
        {title:"1. Confirm glomerular injury",body:"Urine microscopy for dysmorphic RBCs and RBC casts; quantify albumin/protein with ACR or PCR."},
        {title:"2. Assess urgency",body:"Trend creatinine/eGFR, urine output, potassium, bicarbonate, pulmonary symptoms, severe hypertension and fluid overload."},
        {title:"3. Define the syndrome",body:"Nephritic, nephrotic, mixed, rapidly progressive, or isolated urinary abnormality."},
        {title:"4. Search for cause",body:"Infection, systemic autoimmune disease, drug exposure, monoclonal protein, malignancy and hereditary disease."},
        {title:"5. Target tests",body:"Complement, autoimmune tests, infection studies and paraprotein tests selected by phenotype."},
        {title:"6. Biopsy selectively",body:"Obtain tissue when it establishes diagnosis, activity, chronicity, prognosis or treatment—without delaying life-saving therapy in a compatible emergency."}
      ]))}
      ${section("Initial test matrix",table(["Domain","Core tests","Question answered"],[
        ["Urine","Dipstick, microscopy, ACR/PCR; culture if infection suspected","Is bleeding glomerular? How much protein? Is pyuria or infection present?"],
        ["Kidney function","Creatinine/eGFR, urea, electrolytes, bicarbonate, serial values","How rapidly is function changing? Are dialysis indications developing?"],
        ["Blood count / inflammation","CBC, CRP/ESR, blood film if TMA suspected","Anemia, infection, eosinophilia, thrombocytopenia or systemic inflammation?"],
        ["Complement","C3 and C4, sometimes CH50/AP50","Immune-complex or complement-pathway disease?"],
        ["Autoimmune","ANA, anti-dsDNA, PR3/MPO-ANCA, anti-GBM","Lupus, AAV or anti-GBM disease?"],
        ["Infection","Cultures when febrile/endocarditis; HBV, HCV, HIV; streptococcal evidence when relevant","Active or antecedent infectious trigger?"],
        ["Monoclonal protein","Serum/urine immunofixation and serum free light chains","MGRS, myeloma, cryoglobulinemia or AL amyloid?"],
        ["Imaging","Kidney ultrasound; chest imaging for suspected hemorrhage","Kidney size/obstruction and pulmonary-renal emergency?"]
      ]))}
      ${callout("Same-day nephrology red flags","Hemoptysis or diffuse alveolar hemorrhage, rapidly rising creatinine, oliguria/anuria, severe hyperkalemia, pulmonary edema, malignant hypertension, suspected anti-GBM disease or severe ANCA vasculitis.","danger")}
    `),

    "gn-biopsy":M("Kidney Biopsy & Pattern Recognition","Chapter 5 · pathology","Most glomerular diagnoses require integration of light microscopy, immunofluorescence or immunohistochemistry, and electron microscopy with the clinical phenotype.",["LM","IF / IHC","EM"],()=>`
      ${section("Three complementary views",table(["Modality","Main question","Examples"],[
        ["Light microscopy","Is there proliferation, necrosis, crescents, capillary-wall thickening, segmental sclerosis or amyloid?","Endocapillary proliferation in infection-related GN; segmental sclerosis in FSGS; crescents in RPGN."],
        ["IF / IHC","Which immunoglobulins, light chains and complement components are deposited, and in what pattern?","Linear IgG in anti-GBM; mesangial IgA; granular IgG/C3 in MN; pauci-immune AAV."],
        ["Electron microscopy","Where are deposits, and what happened to GBM and podocytes?","Subepithelial deposits in MN; humps in infection-related GN; foot-process effacement in MCD/FSGS; dense deposits in DDD."]
      ]))}
      ${section("High-yield IF and complement matrix",table(["Pattern","Think first","Next investigation"],[
        ["Linear IgG","Anti-GBM disease","Serum anti-GBM and pulmonary hemorrhage assessment."],
        ["Pauci-immune","ANCA-associated or ANCA-negative small-vessel vasculitis","PR3-ANCA, MPO-ANCA and systemic vasculitis assessment."],
        ["IgA-dominant mesangial","IgA nephropathy, IgA vasculitis, IgA-dominant infection-related GN","Look for purpura, liver disease, infection and secondary IgA causes."],
        ["Full-house immune deposits","Lupus nephritis most typical","ANA, anti-dsDNA, C3/C4 and systemic lupus assessment."],
        ["C3 dominant with low C3","Infection-related GN or C3 glomerulopathy","Repeat complement after infection; alternative-pathway work-up if persistent."],
        ["Monoclonal light chain or single Ig subtype","MGRS or hematologic malignancy","Immunofixation, free light chains and hematology referral."],
        ["Minimal staining + diffuse foot-process effacement","MCD or primary FSGS","Confirm nephrotic phenotype and exclude secondary/genetic FSGS."]
      ]))}
      ${callout("When biopsy may be bypassed","Selected examples include a typical child with new steroid-sensitive nephrotic syndrome or an adult with nephrotic syndrome and strongly positive anti-PLA2R supporting primary membranous nephropathy. Atypical features, reduced GFR or uncertainty favor biopsy.","success")}
    `),

    "gn-management":M("General Management & Immunosuppression Safety","Chapter 5 · management","Supportive therapy begins immediately. Immunosuppression is disease-specific, risk-based and often biopsy-directed—not a universal treatment for all glomerular disease.",["Sodium + diuretics","RAS blockade","Infection screening","Emergency support"],()=>`
      ${section("Supportive care matrix",table(["Problem","Management principle","Caution"],[
        ["Edema / sodium retention","Dietary sodium restriction and loop diuretic titrated to congestion","Avoid excessive volume depletion and AKI."],
        ["Proteinuria / hypertension","ACE inhibitor or ARB when appropriate; optimize BP","Check creatinine and potassium; avoid dual ACEi-ARB therapy."],
        ["CKD progression risk","Smoking cessation, healthy weight, exercise, diabetes and cardiovascular risk control","Apply CKD and disease-specific guidance."],
        ["Dyslipidemia","Statin according to CKD and cardiovascular-risk indications","Fibrates need caution in reduced GFR; treatment is not automatic for every GN."],
        ["Thrombosis","Assess nephrotic syndrome, albumin, bleeding risk and disease-specific risk, especially MN","Do not anticoagulate solely because proteinuria exists."],
        ["Infection risk","Vaccination; screen HBV/HCV/HIV/TB as appropriate; prophylaxis for selected regimens","Immunosuppression can reactivate latent infection."],
        ["Kidney failure","Treat hyperkalemia, acidosis, pulmonary edema and uremic complications; dialyze when indicated","Dialysis supports the patient but does not treat the immune cause."]
      ]))}
      ${section("When immune treatment is appropriate",bullets([
        "Disease-specific and risk-based; often guided by biopsy activity and chronicity.",
        "Active infection should be excluded or controlled whenever possible.",
        "Necrotizing crescentic GN, pulmonary hemorrhage and rapidly falling GFR may justify treatment before biopsy results.",
        "Advanced irreversible scarring reduces benefit and increases toxicity."
      ]))}
      ${callout("Pre-treatment safety bundle","Confirm pregnancy status when relevant; screen infections; review vaccines; document CBC, kidney/liver function and immunoglobulins; consider Pneumocystis prophylaxis, gastric/bone protection and fertility preservation according to regimen.","warning")}
    `),

    "gn-mcd-fsgs":M("Minimal Change Disease & FSGS","Chapter 5 · podocytopathies","MCD is a clinicopathologic podocytopathy; FSGS is a histologic pattern that must be classified as primary, secondary/adaptive, genetic or undetermined before immunosuppression.",["Foot-process effacement","Primary vs secondary","Biopsy in adults"],()=>`
      ${section("Minimal change disease",table(["Feature","Key point"],[
        ["Presentation","Abrupt nephrotic syndrome; hematuria or AKI can occur, especially in adults."],
        ["Epidemiology","Most common nephrotic syndrome cause in young children; less common but important in adults."],
        ["LM / IF / EM","Glomeruli normal or near-normal; IF negative/nonspecific; diffuse foot-process effacement without deposits."],
        ["Associations","Idiopathic, NSAIDs, Hodgkin lymphoma, infections and allergic/immune triggers."],
        ["Diagnosis","Typical children often do not need initial biopsy; adults generally require biopsy."],
        ["Treatment","Glucocorticoids are standard first therapy in adults; steroid-sparing options depend on relapse, dependence, contraindication and response."]
      ]))}
      ${section("FSGS classification",table(["Category","Typical clues","Treatment implication"],[
        ["Primary FSGS","Nephrotic syndrome, diffuse foot-process effacement, no secondary driver","High-dose glucocorticoid first-line; CNI for steroid-resistant disease."],
        ["Secondary/adaptive","Obesity, reduced nephron mass, reflux, hypertension, drugs or viral disease; often subnephrotic","Treat cause and use supportive antiproteinuric therapy; avoid routine immunosuppression."],
        ["Genetic","Young age, family history, syndromic features or steroid resistance","Genetic counseling/testing; immunosuppression usually ineffective."],
        ["Undetermined cause","FSGS lesion without nephrotic syndrome or convincing primary features","Supportive care; no routine immunosuppression."]
      ]))}
      ${callout("Two exam corrections","Adult MCD is not diagnosed from selective proteinuria alone, and FSGS is not uniformly steroid resistant. The major safety error is immunosuppressing secondary or genetic FSGS.","danger")}
    `),

    "gn-membranous":M("Membranous Nephropathy","Chapter 5 · nephrotic disease","Membranous nephropathy produces subepithelial immune deposits and often nephrotic syndrome. Anti-PLA2R helps identify primary disease and monitor immunologic response.",["Anti-PLA2R","Subepithelial deposits","Risk-based therapy"],()=>`
      ${section("Core profile",table(["Feature","Key point"],[
        ["Presentation","Nephrotic syndrome or asymptomatic proteinuria; microscopic hematuria may occur."],
        ["Primary biomarker","Anti-PLA2R antibodies in most primary cases; other antigens require expert interpretation."],
        ["Secondary causes","Malignancy, HBV/HCV, autoimmune disease, infection and drugs such as NSAIDs."],
        ["LM","Diffuse capillary-wall/GBM thickening; spikes may appear on silver stain."],
        ["IF","Granular IgG and C3 along capillary loops."],
        ["EM","Subepithelial electron-dense deposits with intervening GBM material."],
        ["Complications","Venous thromboembolism, infection, AKI and progressive CKD in higher-risk disease."]
      ]))}
      ${section("Risk-based management",table(["Risk context","Current approach"],[
        ["All patients","Optimal supportive care and evaluation for secondary causes."],
        ["Nephrotic syndrome + positive anti-PLA2R","Biopsy may not be essential, but reduced GFR, discordance or another possible disease favor biopsy."],
        ["Low risk","Observation and supportive therapy; spontaneous remission can occur."],
        ["Progression risk","Rituximab, cyclophosphamide plus glucocorticoids, or a CNI-based strategy according to risk and comorbidity."],
        ["Monitoring","Proteinuria, albumin, eGFR and serial anti-PLA2R."],
        ["Thrombosis prevention","Individualize using serum albumin, disease risk and bleeding risk."]
      ]))}
      ${callout("Correction","Glucocorticoid monotherapy is not standard primary MN treatment. Modern therapy is selected by progression risk.","warning")}
    `),

    "gn-infection":M("Infection-Related Glomerulonephritis","Chapter 5 · immune-complex disease","The infectious source may precede GN or remain active. Eradicate infection and provide supportive care; immunosuppression during active infection can be dangerous.",["Low C3 often","Subepithelial humps","Treat the source"],()=>`
      ${section("Clinical settings",table(["Setting","Typical pattern"],[
        ["Classic post-streptococcal GN","Nephritic syndrome after pharyngitis or impetigo; low C3; elevated streptococcal antibodies; often children."],
        ["Staphylococcal infection-related GN","Often during active infection in older adults or people with diabetes; can be IgA-dominant."],
        ["Endocarditis-associated GN","Hematuria/proteinuria, low complement, systemic infection, positive cultures and valve infection."],
        ["Shunt or deep-seated infection GN","Persistent immune-complex drive until infected source is eradicated."],
        ["Virus-associated GN","HBV, HCV and HIV can produce distinct lesions requiring virus-specific therapy."]
      ]))}
      ${section("Pathology and management",table(["Domain","Classic finding / action"],[
        ["LM","Diffuse endocapillary hypercellularity with neutrophils."],
        ["IF","Coarse granular C3 with or without IgG/IgA."],
        ["EM","Subepithelial humps may be present."],
        ["Complement","C3 commonly low; persistent low complement requires reassessment."],
        ["Treatment","Culture when feasible, eradicate infection, manage edema/HTN/AKI/electrolytes, and avoid routine immunosuppression during active infection."]
      ]))}
      ${callout("Prognostic concern","Older age, persistent infection, severe AKI, nephrotic-range proteinuria, crescents and chronic scarring predict worse kidney outcomes.","warning")}
    `),

    "gn-rpgn":M("Rapidly Progressive & Crescentic GN","Chapter 5 · emergency pattern","RPGN is rapid kidney-function loss with an active glomerular sediment. Crescents reflect severe capillary injury; immunofluorescence divides the emergency into actionable categories.",["Days to weeks","Active sediment","Urgent biopsy and treatment"],()=>`
      ${section("RPGN categories",table(["Category","IF pattern","Common causes","Treatment direction"],[
        ["Type I: anti-GBM","Linear IgG","Anti-GBM disease, with pulmonary hemorrhage when lungs involved","Urgent plasma exchange + glucocorticoid + cyclophosphamide in appropriate patients."],
        ["Type II: immune-complex","Granular deposits","Lupus, infection-related GN, IgA nephropathy/vasculitis, cryoglobulinemia","Treat underlying disease; immunosuppression depends on cause and activity."],
        ["Type III: pauci-immune","Little or no staining","PR3/MPO-ANCA vasculitis or ANCA-negative vasculitis","Rituximab or cyclophosphamide with glucocorticoid reduction/avacopan; maintenance after remission."],
        ["Double-positive","Anti-GBM plus ANCA","Overlap syndrome","Treat urgently as anti-GBM initially, then maintain as AAV because relapse risk persists."]
      ]))}
      ${section("Immediate actions",numbered([
        "Send creatinine/electrolytes, urine microscopy and protein quantification immediately.",
        "Send anti-GBM, PR3-ANCA, MPO-ANCA, ANA/anti-dsDNA, C3/C4, infection and monoclonal studies as indicated.",
        "Arrange urgent kidney biopsy unless contraindicated; do not delay life-saving therapy in a rapidly deteriorating compatible syndrome.",
        "Assess for diffuse alveolar hemorrhage: dyspnea, falling hemoglobin, hypoxemia and diffuse infiltrates; hemoptysis can be absent.",
        "Treat AKI complications and coordinate nephrology, critical care, respiratory and apheresis services."
      ]))}
      ${callout("Do not use crescent percentage alone","Prognosis also depends on normal glomeruli, global sclerosis, interstitial fibrosis/tubular atrophy, baseline GFR and response to treatment.","info")}
    `),

    "gn-anca-antigbm":M("ANCA Vasculitis & Anti-GBM Disease","Chapter 5 · pulmonary-renal syndromes","Both can cause necrotizing crescentic GN and alveolar hemorrhage, but IF pattern, serology, relapse behavior and plasma-exchange indications differ.",["Pauci-immune vs linear","Pulmonary hemorrhage","Treat before delay"],()=>`
      ${section("ANCA-associated vasculitis",table(["Aspect","High-yield point"],[
        ["Diseases","Granulomatosis with polyangiitis, microscopic polyangiitis and eosinophilic granulomatosis with polyangiitis."],
        ["Serology","PR3-ANCA and MPO-ANCA support diagnosis, but phenotype and biopsy remain important."],
        ["Kidney pathology","Pauci-immune focal necrotizing and crescentic GN."],
        ["Induction","Glucocorticoids with rituximab or cyclophosphamide; avacopan can reduce glucocorticoid exposure in selected patients."],
        ["Plasma exchange","Selected severe kidney failure/rapid rise, dialysis dependence, hypoxemic alveolar hemorrhage, or ANCA/anti-GBM overlap—not routine for everyone."],
        ["Maintenance","Usually rituximab or azathioprine with low-dose glucocorticoid after remission."]
      ]))}
      ${section("Anti-GBM disease",table(["Feature","Key point"],[
        ["Mechanism","Autoantibodies target the alpha-3 chain of type IV collagen in glomerular and alveolar basement membranes."],
        ["Presentation","Severe RPGN; pulmonary hemorrhage may cause hypoxemia and falling hemoglobin."],
        ["Biopsy","Necrotizing crescentic GN with strong linear IgG along GBM."],
        ["Treatment","Plasma exchange plus glucocorticoids and cyclophosphamide; continue exchange until antibody is undetectable under specialist protocol."],
        ["Relapse","Uncommon after antibodies disappear, except in double-positive disease."],
        ["Transplant","Defer until anti-GBM antibodies remain undetectable for at least 6 months."]
      ]))}
      ${callout("Pulmonary hemorrhage can be occult","Absence of hemoptysis does not exclude diffuse alveolar hemorrhage. Falling hemoglobin, new bilateral opacities and hypoxemia in RPGN require urgent investigation.","danger")}
    `),

    "gn-mpgn-c3g":M("MPGN Pattern & C3 Glomerulopathy","Chapter 5 · mechanism-first classification","MPGN is a pattern of mesangial/endocapillary proliferation and capillary-wall remodeling—not a single disease. Classify by IF mechanism and find the driver.",["Pattern not disease","Immune-complex","Monoclonal","Complement-mediated"],()=>`
      ${section("Modern classification",table(["Category","Mechanism / examples","Required search"],[
        ["Immune-complex-mediated GN","Chronic infection, autoimmune disease, cryoglobulinemia, polyclonal immune complexes","HBV/HCV, endocarditis/infection, ANA/complement and cryoglobulins."],
        ["Monoclonal immunoglobulin-mediated GN","MGRS, lymphoma or plasma-cell clone","Serum/urine immunofixation, free light chains, hematology and biopsy typing."],
        ["Complement-mediated GN / C3G","Alternative pathway dysregulation; C3 GN or dense deposit disease","Exclude infection; complement studies, autoantibodies and genetics in a specialist center."],
        ["TMA / mimics","Thrombotic microangiopathy and chronic endothelial injury can produce double contours","Platelets, hemolysis tests, BP and drug/pregnancy/systemic assessment."]
      ]))}
      ${section("Management principles",bullets([
        "Treat the underlying infection, autoimmune process or monoclonal clone when identified.",
        "Use supportive antiproteinuric and blood-pressure therapy in indolent disease.",
        "Active idiopathic immune-complex GN may need glucocorticoid plus another immunosuppressant according to severity and biopsy activity.",
        "Moderate-to-severe C3G has traditionally used mycophenolate plus glucocorticoid; complement inhibitors and trials are increasingly important in specialist care.",
        "Recurrence after transplant is possible, especially in complement-mediated and monoclonal disease."
      ]))}
      ${callout("Correction","The old MPGN type I/II/III scheme and routine aspirin-dipyridamole treatment are obsolete. Identify and treat the mechanism.","warning")}
    `),

    "gn-iga":M("IgA Nephropathy","Chapter 5 · mesangial IgA disease","IgA nephropathy classically causes synpharyngitic hematuria. Diagnosis requires biopsy, and current care targets long-term proteinuria reduction and GFR preservation.",["Synpharyngitic hematuria","Mesangial IgA","MEST-C","Risk-based care"],()=>`
      ${section("Core profile",table(["Feature","Key point"],[
        ["Presentation","Visible hematuria during or within days of a respiratory/GI infection, persistent microscopic hematuria, proteinuria, hypertension or CKD."],
        ["Diagnosis","Kidney biopsy with dominant or co-dominant mesangial IgA, usually with C3."],
        ["Pathology score","Oxford MEST-C: mesangial and endocapillary hypercellularity, segmental sclerosis, tubular atrophy/interstitial fibrosis and crescents."],
        ["Secondary causes","IgA vasculitis, chronic liver disease, infection, inflammatory bowel disease, autoimmune disease and HIV/hepatitis."],
        ["Complement","Serum complement is usually normal; low complement suggests another or additional process."],
        ["Differential","Post-infectious GN occurs after a latency; IgA vasculitis adds systemic features such as purpura and abdominal/joint symptoms."]
      ]))}
      ${section("Risk and treatment framework",table(["Decision point","Current approach"],[
        ["Biopsy threshold","Consider in adults with proteinuria ≥0.5 g/day when IgA nephropathy is possible and biopsy is safe."],
        ["Risk definition","Proteinuria ≥0.5 g/day signals progression risk; integrate eGFR, proteinuria, MEST-C and validated tools."],
        ["Treatment goal","Aim for proteinuria <0.5 g/day and ideally <0.3 g/day while preserving long-term GFR."],
        ["Foundation","Maximally tolerated ACE inhibitor or ARB unless contraindicated, lifestyle and BP optimization."],
        ["Additional protection","SGLT2 inhibitor for progression risk; sparsentan where approved as an alternative that must not be combined with another RAS inhibitor."],
        ["Immune-targeted therapy","Targeted-release budesonide in eligible at-risk patients; reduced-dose systemic glucocorticoid only with careful selection and infection prophylaxis."],
        ["RPGN variant","Cyclophosphamide plus systemic glucocorticoid; crescents alone without rapid GFR loss are not RPGN."]
      ]))}
      ${callout("Corrections","Long-term antibiotics and routine omega-3 are not standard treatment. Tonsillectomy is not recommended as IgA nephropathy therapy in non-Japanese patients.","info")}
    `),

    "gn-col4":M("COL4-Related Disease & Alport Spectrum","Chapter 5 · inherited glomerular hematuria","Persistent familial glomerular hematuria from COL4A3, COL4A4 or COL4A5 variants spans isolated hematuria to classical Alport syndrome. A thin GBM is not automatically benign.",["Familial hematuria","Hearing + eye clues","Genetic diagnosis"],()=>`
      ${section("Alport spectrum",table(["Feature","Key point"],[
        ["Inheritance","X-linked COL4A5 is common; autosomal recessive and autosomal dominant COL4A3/4 disease also occur."],
        ["Kidney findings","Persistent microscopic hematuria, later proteinuria, hypertension and progressive CKD in higher-risk genotypes."],
        ["Extrarenal findings","Sensorineural hearing loss; anterior lenticonus and retinal changes."],
        ["EM","Early GBM thinning; later irregular thickening, splitting and lamellation—the basket-weave appearance."],
        ["Diagnosis","Family history, phenotype and genetic testing; biopsy can support but genetics guides family counseling."],
        ["Management","Early RAS blockade when albuminuria/proteinuria appears, sometimes earlier in high-risk disease; monitor BP, ACR, eGFR, hearing and eyes."],
        ["Transplant","Kidney transplantation is effective; rare post-transplant anti-GBM nephritis can occur."]
      ]))}
      ${section("Thin basement membrane lesion",bullets([
        "An isolated thin GBM on EM is a morphologic finding, not proof of a harmless course.",
        "Some heterozygous COL4A3/4 carriers remain stable; others develop proteinuria, FSGS and CKD.",
        "Family history of kidney failure, hearing loss or proteinuria should trigger genetics-focused assessment."
      ]))}
      ${callout("Correction","Alport syndrome is not benign familial hematuria. Modern care includes genetic diagnosis, family screening and early nephroprotection.","warning")}
    `),

    "gn-amyloid":M("Renal Amyloidosis","Chapter 5 · deposition disease","Amyloidosis is a protein-folding disorder and a glomerulopathy rather than inflammatory GN. Tissue confirmation and precise fibril typing are essential because treatments differ completely.",["Congo red","Type the fibril","AL vs AA"],()=>`
      ${section("Major renal amyloid types",table(["Type","Source / setting","Kidney and treatment implication"],[
        ["AL amyloidosis","Monoclonal immunoglobulin light chain from a plasma-cell or B-cell clone","Proteinuria/nephrotic syndrome and CKD; urgent hematology and clone-directed therapy."],
        ["AA amyloidosis","Serum amyloid A driven by chronic inflammatory or infectious disease, including FMF","Control the inflammatory driver; colchicine prevents/treats FMF-associated AA amyloid."],
        ["Beta-2 microglobulin","Long dialysis vintage, especially older low-flux exposure","Usually osteoarticular; high-flux dialysis and transplantation reduce burden."],
        ["Hereditary / other","ATTR and rarer proteins","Genetic and organ-specific assessment; do not assume all amyloid is AL."]
      ]))}
      ${section("Diagnostic sequence",numbered([
        "Demonstrate amyloid in tissue using Congo red with apple-green birefringence under polarized light.",
        "Type the amyloid, preferably by mass spectrometry or validated expert immunohistochemistry.",
        "For suspected AL, obtain serum and urine immunofixation plus serum free light chains and assess cardiac involvement urgently.",
        "Assess renal burden with proteinuria, albumin and eGFR while evaluating other organs according to amyloid type."
      ]))}
      ${callout("Diagnostic safety point","A monoclonal protein does not prove AL amyloidosis, and negative routine electrophoresis alone does not exclude it. Tissue typing and sensitive monoclonal studies are essential.","danger")}
    `),

    "gn-summary":M("Glomerular Disease: High-Yield Matrix","Chapter 5 · final integration","Use this final page to connect urine sediment, complement, IF pattern, EM location and classic disease. It also corrects high-risk legacy shortcuts.",["Pattern matrix","Legacy corrections","Emergency rule"],()=>`
      ${section("One-page diagnostic memory aid",table(["Finding","Think first"],[
        ["Dysmorphic RBCs, acanthocytes or RBC casts","Glomerular bleeding."],
        ["Linear IgG on IF","Anti-GBM disease."],
        ["Pauci-immune necrotizing crescentic GN","ANCA-associated vasculitis until proved otherwise."],
        ["Low C3 + infection","Infection-related GN."],
        ["Low C3 persists after infection","C3 glomerulopathy or ongoing immune-complex disease."],
        ["Low C3 + low C4","Lupus, cryoglobulinemia or classical-pathway immune-complex disease."],
        ["Normal complement + synpharyngitic hematuria","IgA nephropathy."],
        ["Normal complement + pulmonary hemorrhage","ANCA vasculitis or anti-GBM disease."],
        ["Subepithelial humps","Infection-related GN."],
        ["Subepithelial deposits + granular capillary-loop IgG","Membranous nephropathy."],
        ["Diffuse foot-process effacement + normal LM","Minimal change disease; verify EM."],
        ["Family hematuria + hearing loss","COL4-related Alport spectrum."],
        ["Congo red + apple-green birefringence","Amyloid; then type the fibril protein."]
      ]))}
      ${section("Legacy statements corrected",table(["Unsafe shortcut","Current interpretation"],[
        ["All GN receives steroids","Immunosuppression is disease-specific and can be dangerous in infection, secondary FSGS or irreversible scarring."],
        ["Selective proteinuria diagnoses MCD","Adult MCD requires biopsy; typical children are managed clinically unless atypical or resistant."],
        ["Mild MN gets steroids","Steroid monotherapy is ineffective; use supportive and risk-based therapy."],
        ["FSGS is steroid resistant","Primary FSGS may respond; secondary/genetic/undetermined FSGS should not receive routine immunosuppression."],
        ["Plasma exchange for every crescentic GN","Essential in anti-GBM, selected severe AAV, not routine for all immune-complex crescentic GN."],
        ["MPGN types I/II/III guide treatment","MPGN is a pattern; classify immune-complex, monoclonal or complement-mediated disease."],
        ["Thin GBM and Alport are benign","COL4 disease is a spectrum with possible progression; use genetics and early nephroprotection."],
        ["All amyloid is AL","Type the fibril precisely before treatment."]
      ]))}
      ${callout("Clinical priority","Rapidly rising creatinine with RBC casts—especially with pulmonary symptoms—is a nephrology emergency. Send ANCA and anti-GBM serology, arrange urgent biopsy and do not permit administrative delay to postpone appropriate therapy.","danger")}
      ${routeLinks([["gn-pattern-lab","Open the GN pattern interpreter"],["clinical-cases","Apply to clinical cases"],["flashcards","Review with flashcards"],["quiz","Take the scored quiz"]])}
    `),
    ...
    "gn-pattern-lab":M("GN Pattern Interpreter","Interactive diagnostic reasoning","Combine clinical syndrome, complement, immunofluorescence, electron-microscopy location and systemic clues. The tool ranks pattern families and flags emergencies without pretending to replace biopsy.",["Syndrome + complement","IF + EM","Emergency flags"],()=>`
      ${section("Build a glomerular pattern",`<div class="tool-panel" id="gn-pattern-tool"><div class="tool-controls"><label class="field"><span>Clinical syndrome</span><select id="g-syndrome"><option value="nephritic">Acute nephritic</option><option value="nephrotic">Nephrotic</option><option value="mixed">Mixed nephritic-nephrotic</option><option value="rpgn">Rapidly progressive</option><option value="isolated">Isolated hematuria/proteinuria</option></select></label><label class="field"><span>Complement</span><select id="g-complement"><option value="normal">Normal</option><option value="lowc3">Low C3, C4 normal</option><option value="lowboth">Low C3 and C4</option><option value="unknown">Unknown</option></select></label><label class="field"><span>IF pattern</span><select id="g-if"><option value="unknown">Not available</option><option value="linear">Linear IgG</option><option value="pauci">Pauci-immune</option><option value="iga">IgA-dominant mesangial</option><option value="fullhouse">Full-house</option><option value="c3">C3-dominant</option><option value="granularcap">Granular capillary-loop IgG/C3</option><option value="minimal">Minimal staining</option><option value="monoclonal">Monoclonal light chain / single Ig</option></select></label><label class="field"><span>EM / pathology clue</span><select id="g-em"><option value="unknown">Not available</option><option value="humps">Subepithelial humps</option><option value="subepithelial">Subepithelial deposits</option><option value="effacement">Diffuse foot-process effacement</option><option value="dense">Dense deposits</option><option value="basket">Basket-weave GBM</option><option value="amyloid">Congo red / fibrils</option><option value="crescent">Necrosis / crescents</option></select></label><label class="field"><span>Clinical clue</span><select id="g-clue"><option value="none">No dominant clue</option><option value="infection">Active/recent infection</option><option value="synpharyngitic">Synpharyngitic hematuria</option><option value="pulmonary">Pulmonary hemorrhage / hemoptysis</option><option value="purpura">Purpura / systemic vasculitis</option><option value="pla2r">Positive anti-PLA2R</option><option value="family">Family hematuria + hearing loss</option><option value="monoclonal">Monoclonal protein / plasma-cell clue</option></select></label><label class="field"><span>Kidney trajectory</span><select id="g-trajectory"><option value="stable">Stable / slow</option><option value="falling">Rapidly rising creatinine</option><option value="dialysis">Dialysis-dependent / severe AKI</option></select></label></div><div class="tool-output" id="gn-pattern-output"></div></div>`)}
      ${callout("How to use it","Make a prediction before changing each field. The output gives a ranked pattern family, the decisive next test, and emergency escalation—not a final patient diagnosis.","purple")}
    `),

    "clinical-cases":M("Progressive Clinical Cases","Interactive revision","Work through renal physiology, investigation, proteinuria, hematuria, and glomerular disease cases. Reveal diagnosis, reasoning, next-step framework, and exam pearl in stages.",["24 cases","Progressive reveals","Mixed topics"],()=>`
    ...
    flashcards:M("Flashcards","Interactive revision","Flip 104 cards, filter by topic, shuffle, and mark cards as known or needing review. Progress is stored only in this browser.",["104 cards","6 categories","Local review status"],()=>`
    ...
    quiz:M("Scored Renal Quiz","Interactive revision","Complete 60 single-best-answer questions. Every response includes an explanation, and progress remains available after closing the browser.",["60 questions","Immediate explanations","Saved progress"],()=>`
    ...
    sources:M("Sources, Scope & Original PDFs","Reference library","Open any of the five original chapters inside the site or in a new tab. The interactive content reorganizes the supplied PDFs without replacing their full tables, references, pathology detail, or clinical cautions.",["5 embedded PDFs","Chapter references","Educational scope"],()=>`
      ${section("Original chapter viewer",`<div class="tool-panel" id="source-tool"><div class="tool-controls"><label class="field"><span>Select PDF</span><select id="pdf-select"><option value="01-renal-principles.pdf">01 · Renal Principles</option><option value="02-renal-investigations.pdf">02 · Renal Investigations</option><option value="03-proteinuria.pdf">03 · Proteinuria</option><option value="04-hematuria.pdf">04 · Hematuria</option><option value="05-glomerulonephritis.pdf">05 · Glomerulonephritis</option></select></label><div class="source-open"><a class="primary-button" id="pdf-open" href="01-renal-principles.pdf" target="_blank" rel="noopener">Open selected PDF in a new tab</a></div></div><object class="pdf-frame" id="pdf-frame" data="01-renal-principles.pdf" type="application/pdf"><p>Your browser cannot embed the PDF. Use the open button above.</p></object></div>`)}
      ${section("Chapter sources",`<div class="source-grid">
        ...
        <article class="source-card"><h3>05 · Glomerulonephritis</h3><p>Clinical syndromes, immune mechanisms, biopsy pattern recognition, major glomerular diseases, RPGN, current management, and legacy corrections.</p><a href="05-glomerulonephritis.pdf" target="_blank" rel="noopener">Open PDF</a></article>
      </div>`)}
      ${section("Key external references named in the chapters",bullets([
        ...
        '<a href="https://kdigo.org/wp-content/uploads/2017/02/KDIGO-Glomerular-Diseases-Guideline-2021-English.pdf" target="_blank" rel="noopener">KDIGO 2021 Glomerular Diseases Guideline</a>',
        '<a href="https://kdigo.org/wp-content/uploads/2024/05/KDIGO-2024-ANCA-Vasculitis-Guideline-Update.pdf" target="_blank" rel="noopener">KDIGO 2024 ANCA Vasculitis Update</a>',
        '<a href="https://kdigo.org/wp-content/uploads/2024/08/KDIGO-2025-IgAN-IgAV-Guideline.pdf" target="_blank" rel="noopener">KDIGO 2025 IgA Nephropathy / IgA Vasculitis Guideline</a>',
        '<a href="https://www.niddk.nih.gov/health-information/kidney-disease/glomerular-diseases" target="_blank" rel="noopener">NIDDK: Glomerular Diseases</a>',
        '<a href="https://www.niddk.nih.gov/health-information/kidney-disease/amyloidosis" target="_blank" rel="noopener">NIDDK: Amyloidosis and Kidney Disease</a>',
      ...
      ${section("Scope and safety",cards([
        {icon:"PDF",title:"Source fidelity",body:"<p>The website preserves the chapter logic, disease distinctions and key correction boxes while reorganizing material into connected study modules.</p>"},
        {icon:"↻",title:"Current-practice notes",body:"<p>Drug access, approval, monitoring and local protocols vary. The supplied chapter incorporates 2024 ANCA and 2025 IgA updates where they supersede older guidance.</p>"},
        {icon:"!",title:"Educational use",body:"<p>This is revision material, not unsupervised prescribing. Pulmonary-renal syndrome, RPGN, severe hyperkalemia, pulmonary edema, anuria and infected obstruction require urgent specialist care.</p>"}
      ]))}
    `)
  };
  ...
  const cases = [
    ...
    {title:"Synpharyngitic hematuria",tag:"Glomerular disease",stem:"A 24-year-old develops cola-colored urine during an upper-respiratory infection. BP is mildly elevated, complement is normal, and urine shows dysmorphic RBCs with moderate proteinuria.",clues:["Hematuria during infection","Normal complement","Dysmorphic RBCs","Young adult"],question:"Which disease pattern is most likely and what confirms it?",diagnosis:"IgA nephropathy is likely; kidney biopsy confirms dominant or co-dominant mesangial IgA.",reasoning:"The timing is synpharyngitic rather than delayed, complement is usually normal, and the sediment is glomerular.",action:"Quantify proteinuria, trend eGFR and BP, exclude secondary IgA causes, and arrange nephrology/biopsy according to proteinuria and risk.",pearl:"IgA nephropathy occurs during or shortly after mucosal infection; post-infectious GN classically follows a latency."},
    {title:"Low C3 after skin infection",tag:"Glomerular disease",stem:"A 10-year-old develops edema, hypertension and dark urine two weeks after impetigo. C3 is low and urine shows RBC casts.",clues:["Delayed after impetigo","Low C3","Nephritic syndrome","RBC casts"],question:"What is the leading diagnosis and key management principle?",diagnosis:"Classic post-streptococcal infection-related GN.",reasoning:"The delayed nephritic syndrome, low C3 and active sediment are classic. C3 should recover; persistent hypocomplementemia prompts reassessment.",action:"Support edema, BP, AKI and electrolytes, treat any active infection, and avoid routine immunosuppression.",pearl:"Antibiotics eradicate infection but do not instantly reverse established immune injury."},
    {title:"Nephrotic adult with anti-PLA2R",tag:"Glomerular disease",stem:"A 46-year-old has edema, albumin 2.1 g/dL, proteinuria 7 g/day, bland sediment and positive anti-PLA2R antibodies.",clues:["Nephrotic syndrome","Bland sediment","Anti-PLA2R positive","Adult"],question:"Which diagnosis is likely and how is treatment chosen?",diagnosis:"Primary membranous nephropathy is likely.",reasoning:"Nephrotic syndrome with anti-PLA2R strongly supports primary MN; biopsy may be bypassed in selected typical cases but reduced GFR or discordance favors tissue.",action:"Evaluate secondary causes and thrombosis risk, optimize supportive care, then choose observation or rituximab/cyclophosphamide/CNI strategy according to progression risk.",pearl:"Glucocorticoid monotherapy is not standard treatment for primary MN."},
    {title:"Pulmonary hemorrhage with linear IgG",tag:"Emergency",stem:"A 29-year-old has rapidly rising creatinine, hypoxemia, falling hemoglobin and diffuse pulmonary opacities. Biopsy shows crescents and strong linear IgG along the GBM.",clues:["RPGN","Alveolar hemorrhage","Linear IgG","Crescents"],question:"What is the diagnosis and immediate treatment direction?",diagnosis:"Anti-GBM disease with pulmonary hemorrhage.",reasoning:"Linear GBM IgG plus pulmonary-renal syndrome is diagnostic. Hemoptysis may be absent despite diffuse alveolar hemorrhage.",action:"Start urgent plasma exchange, glucocorticoids and cyclophosphamide under specialist care; do not wait for every confirmatory result.",pearl:"Goodpasture syndrome is best reserved for anti-GBM disease with pulmonary hemorrhage."},
    {title:"Sinus disease, purpura and pauci-immune GN",tag:"Glomerular disease",stem:"A 58-year-old has chronic sinusitis, palpable purpura, mononeuritis and rapidly falling eGFR. Biopsy shows necrotizing crescentic GN with little immune staining.",clues:["ENT disease","Purpura/neuropathy","Pauci-immune biopsy","Rapid renal decline"],question:"Which disease family is most likely?",diagnosis:"ANCA-associated vasculitis, such as granulomatosis with polyangiitis or microscopic polyangiitis.",reasoning:"Pauci-immune necrotizing crescentic GN with systemic small-vessel vasculitis features strongly supports AAV, even if ANCA is negative.",action:"Send PR3/MPO-ANCA and systemic assessment, but begin urgent induction when the compatible disease is rapidly deteriorating.",pearl:"Biopsy and phenotype remain important; ANCA supports but does not replace clinical diagnosis."},
    {title:"Obesity with secondary FSGS",tag:"Glomerular disease",stem:"A 42-year-old with severe obesity has 1.8 g/day proteinuria, preserved albumin and no edema. Biopsy shows perihilar segmental sclerosis and patchy foot-process effacement.",clues:["Subnephrotic proteinuria","No nephrotic syndrome","Obesity","Patchy effacement"],question:"Should routine immunosuppression be used?",diagnosis:"Secondary/adaptive FSGS is more likely than primary FSGS.",reasoning:"The adaptive driver, subnephrotic phenotype and patchy effacement argue against primary circulating-factor FSGS.",action:"Treat the driver, optimize BP and antiproteinuric therapy, and avoid routine immunosuppression.",pearl:"FSGS is a pattern, not one disease; classification determines whether steroids are appropriate."},
    {title:"Family hematuria and hearing loss",tag:"Genetic",stem:"A 19-year-old has lifelong microscopic hematuria, new albuminuria, a maternal family history of kidney failure and sensorineural hearing loss.",clues:["Familial hematuria","Hearing loss","Albuminuria","Young age"],question:"What inherited disease should be investigated?",diagnosis:"COL4-related Alport spectrum.",reasoning:"Persistent familial glomerular hematuria with hearing loss is classic for type IV collagen disease.",action:"Arrange genetics-focused assessment and family screening; monitor BP, ACR and eGFR and start nephroprotection when indicated.",pearl:"A thin basement membrane is not automatically benign."},
    {title:"Nephrotic syndrome with monoclonal protein",tag:"Glomerular disease",stem:"A 68-year-old has nephrotic syndrome, restrictive cardiomyopathy and a small monoclonal spike. Kidney biopsy is Congo-red positive.",clues:["Nephrotic syndrome","Cardiac involvement","Monoclonal protein","Congo red"],question:"Does the monoclonal protein prove AL amyloidosis?",diagnosis:"Amyloidosis is confirmed, but fibril typing is still required.",reasoning:"A monoclonal protein can be incidental; AL, AA, ATTR and other types require different therapy.",action:"Type amyloid by mass spectrometry or validated expert method, obtain sensitive monoclonal studies and urgently assess cardiac involvement.",pearl:"Congo red diagnoses amyloid; it does not identify the amyloid protein."}
  ];
  ...
  const flashcards = [
    ...
    {tag:"Glomerular disease",q:"What is the difference between glomerulonephritis and glomerulopathy?",a:"GN implies inflammatory glomerular injury; glomerulopathy is broader and includes non-inflammatory lesions such as MCD, membranous nephropathy and amyloidosis."},
    {tag:"Glomerular disease",q:"Best urine clue that bleeding is glomerular?",a:"Dysmorphic RBCs, acanthocytes and especially RBC casts, often with proteinuria."},
    {tag:"Glomerular disease",q:"What does a crescent represent?",a:"A severe histologic response to capillary-wall rupture, not one specific disease."},
    {tag:"Glomerular disease",q:"Five pathogenic GN/glomerular patterns?",a:"Immune-complex, anti-GBM, pauci-immune, complement-dominant and podocytopathy."},
    {tag:"Glomerular disease",q:"What IF pattern suggests anti-GBM disease?",a:"Strong linear IgG along the GBM."},
    {tag:"Glomerular disease",q:"What IF pattern suggests ANCA-associated vasculitis?",a:"Pauci-immune: little or no immune staining."},
    {tag:"Glomerular disease",q:"What does full-house IF suggest?",a:"Lupus nephritis most typically."},
    {tag:"Glomerular disease",q:"What does C3-dominant IF with persistent low C3 suggest?",a:"C3 glomerulopathy or ongoing immune-complex disease after infection has been excluded."},
    {tag:"Glomerular disease",q:"What are the three biopsy modalities for GN?",a:"Light microscopy, immunofluorescence/immunohistochemistry and electron microscopy."},
    {tag:"Glomerular disease",q:"Which EM finding is classic for MCD?",a:"Diffuse podocyte foot-process effacement without immune deposits."},
    {tag:"Glomerular disease",q:"Which pathology is classic for membranous nephropathy?",a:"Subepithelial deposits with granular capillary-loop IgG/C3 and GBM thickening."},
    {tag:"Glomerular disease",q:"Which EM finding is classic for infection-related GN?",a:"Subepithelial humps."},
    {tag:"Glomerular disease",q:"What is FSGS?",a:"A histologic pattern of focal and segmental glomerular sclerosis, not a single disease."},
    {tag:"Glomerular disease",q:"Which FSGS category is routinely considered for immunosuppression?",a:"Primary FSGS with a convincing nephrotic syndrome phenotype."},
    {tag:"Glomerular disease",q:"Why avoid immunosuppression in secondary FSGS?",a:"The lesion is adaptive to another driver; steroids add toxicity without treating the mechanism."},
    {tag:"Glomerular disease",q:"Does adult MCD require biopsy?",a:"Usually yes; selective proteinuria alone does not diagnose adult MCD."},
    {tag:"Glomerular disease",q:"Primary biomarker in most primary membranous nephropathy?",a:"Anti-PLA2R antibodies."},
    {tag:"Glomerular disease",q:"Is glucocorticoid monotherapy standard for primary MN?",a:"No. Therapy is risk-based and may use rituximab, cyclophosphamide-glucocorticoid or CNI strategies."},
    {tag:"Glomerular disease",q:"Classic complement pattern in post-infectious GN?",a:"Low C3, which should usually recover; persistent low C3 prompts reassessment."},
    {tag:"Glomerular disease",q:"RPGN type I pattern and cause?",a:"Linear IgG caused by anti-GBM disease."},
    {tag:"Glomerular disease",q:"RPGN type II pattern?",a:"Granular immune-complex deposits."},
    {tag:"Glomerular disease",q:"RPGN type III pattern?",a:"Pauci-immune necrotizing GN, usually ANCA-associated vasculitis."},
    {tag:"Glomerular disease",q:"Can pulmonary hemorrhage occur without hemoptysis?",a:"Yes. Falling hemoglobin, hypoxemia and new bilateral opacities can reveal occult alveolar hemorrhage."},
    {tag:"Glomerular disease",q:"When is plasma exchange essential?",a:"Anti-GBM disease; it is considered selectively in severe AAV and is not routine for every crescentic GN."},
    {tag:"Glomerular disease",q:"What does MPGN mean today?",a:"A pattern requiring classification as immune-complex, monoclonal, complement-mediated or a mimic such as TMA."},
    {tag:"Glomerular disease",q:"Classic timing of IgA nephropathy hematuria?",a:"During or within days of a mucosal infection—synpharyngitic hematuria."},
    {tag:"Glomerular disease",q:"Is kidney biopsy required to diagnose IgA nephropathy?",a:"Yes; IF shows dominant or co-dominant mesangial IgA, usually with C3."},
    {tag:"Glomerular disease",q:"What is the Oxford MEST-C score?",a:"Mesangial and endocapillary hypercellularity, segmental sclerosis, tubular atrophy/interstitial fibrosis and crescents."},
    {tag:"Glomerular disease",q:"Family hematuria plus hearing loss suggests?",a:"COL4-related Alport spectrum."},
    {tag:"Glomerular disease",q:"What is the basket-weave GBM appearance?",a:"Irregular thickening, splitting and lamellation in later COL4-related disease."},
    {tag:"Glomerular disease",q:"How is amyloid demonstrated in tissue?",a:"Congo red staining with apple-green birefringence under polarized light."},
    {tag:"Glomerular disease",q:"Does a monoclonal spike prove AL amyloidosis?",a:"No. Amyloid must be typed because monoclonal gammopathy may be incidental."}
  ];
  ...
  const quiz = [
    ...
    {q:"Which biopsy pattern is most characteristic of anti-GBM disease?",options:["Granular mesangial IgA","Strong linear IgG along GBM","Pauci-immune staining","C3-only deposits"],answer:1,explanation:"Anti-GBM antibodies bind basement-membrane collagen, producing linear IgG staining."},
    {q:"A crescent on kidney biopsy means:",options:["One specific disease","Severe capillary-wall injury response","Always chronic irreversible scarring","Only IgA nephropathy"],answer:1,explanation:"Crescents are a histologic response shared by anti-GBM, AAV and severe immune-complex GN."},
    {q:"Which presentation best defines RPGN?",options:["Stable proteinuria for years","Rapid kidney-function loss over days to weeks with active sediment","Isolated glycosuria","Painless visible hematuria with clots"],answer:1,explanation:"RPGN is a clinical syndrome of rapid decline with glomerular urine findings, often with crescents."},
    {q:"Pauci-immune necrotizing crescentic GN most strongly suggests:",options:["ANCA-associated vasculitis","Membranous nephropathy","Minimal change disease","Amyloidosis"],answer:0,explanation:"AAV is the classic cause of pauci-immune necrotizing crescentic GN."},
    {q:"Subepithelial humps on EM are classically associated with:",options:["Infection-related GN","Anti-GBM disease","FSGS","Alport syndrome"],answer:0,explanation:"Subepithelial humps are a classic infection-related GN finding."},
    {q:"Diffuse foot-process effacement with nearly normal LM suggests:",options:["Minimal change disease","C3 glomerulopathy","Anti-GBM disease","Post-renal bleeding"],answer:0,explanation:"MCD has diffuse foot-process effacement with minimal LM and IF abnormalities."},
    {q:"Which patient is most likely to have secondary/adaptive FSGS?",options:["Nephrotic syndrome with diffuse effacement and no driver","Obesity with subnephrotic proteinuria and perihilar sclerosis","Child with steroid-sensitive nephrotic syndrome","Linear IgG with pulmonary hemorrhage"],answer:1,explanation:"Obesity and an adaptive phenotype suggest secondary FSGS, which is managed by treating the driver and supportive therapy."},
    {q:"Which statement about primary membranous nephropathy is correct?",options:["Steroid monotherapy is standard","Anti-PLA2R can support diagnosis and monitoring","It is always infection-related","It never causes thrombosis"],answer:1,explanation:"Anti-PLA2R is a key biomarker; treatment is risk-based and thrombosis risk can be substantial."},
    {q:"A nephritic syndrome two weeks after impetigo with low C3 suggests:",options:["IgA nephropathy","Post-streptococcal infection-related GN","MCD","Anti-GBM disease"],answer:1,explanation:"The delayed timing and low C3 are classic for post-infectious GN."},
    {q:"The most important first treatment principle in active infection-related GN is:",options:["Immediate universal steroids","Eradicate the infection and provide supportive care","Routine plasma exchange","Long-term anticoagulation"],answer:1,explanation:"Active infection must be treated; immunosuppression is generally avoided unless an exceptional expert-directed indication exists."},
    {q:"Which combination is most urgent for anti-GBM disease?",options:["ACE inhibitor plus statin","Plasma exchange, glucocorticoid and cyclophosphamide","Antibiotics only","Thiazide only"],answer:1,explanation:"Anti-GBM disease requires rapid antibody removal plus suppression of further production."},
    {q:"Absence of hemoptysis in RPGN:",options:["Excludes alveolar hemorrhage","Does not exclude alveolar hemorrhage","Proves MCD","Proves urologic bleeding"],answer:1,explanation:"Alveolar hemorrhage may be occult; falling hemoglobin, hypoxemia and diffuse infiltrates are key clues."},
    {q:"Modern MPGN classification is primarily based on:",options:["Types I, II and III only","Immunofluorescence mechanism and underlying driver","Urine color only","Age alone"],answer:1,explanation:"MPGN is a pattern classified as immune-complex, monoclonal, complement-mediated or a mimic."},
    {q:"Visible hematuria during an upper-respiratory infection with normal complement most suggests:",options:["IgA nephropathy","Post-infectious GN after latency","MCD","Amyloidosis"],answer:0,explanation:"Synpharyngitic hematuria is a hallmark of IgA nephropathy."},
    {q:"Which statement about crescents in IgA nephropathy is correct?",options:["Any crescent means RPGN","Crescents alone without rapid GFR loss do not define RPGN","They exclude IgA nephropathy","They always mandate plasma exchange"],answer:1,explanation:"RPGN requires rapid functional decline; crescent percentage alone is insufficient."},
    {q:"Persistent familial hematuria with hearing loss most suggests:",options:["COL4-related Alport spectrum","Simple cystitis","Membranous nephropathy","Post-infectious GN"],answer:0,explanation:"Hearing and ocular features are classic extrarenal clues to type IV collagen disease."},
    {q:"Congo red with apple-green birefringence identifies:",options:["Amyloid","Crescents","Podocyte effacement","RBC casts"],answer:0,explanation:"Congo red confirms amyloid deposition, after which the fibril protein must be typed."},
    {q:"A monoclonal protein in a patient with amyloid:",options:["Always proves AL type","Still requires tissue typing","Rules out AA amyloid","Makes biopsy unnecessary"],answer:1,explanation:"Monoclonal gammopathy may be incidental; precise amyloid typing is essential."},
    {q:"Which statement about immunosuppression in GN is safest?",options:["Every GN needs steroids","It is disease-specific and may be harmful in active infection or secondary FSGS","Biopsy never matters","Advanced scarring increases treatment benefit"],answer:1,explanation:"Immune treatment must match disease, activity and reversibility while minimizing infection and toxicity."},
    {q:"Rapidly rising creatinine, RBC casts and pulmonary symptoms should trigger:",options:["Routine follow-up in six months","Same-day pulmonary-renal syndrome assessment","Only a urine culture","No serology"],answer:1,explanation:"This is a nephrology emergency requiring ANCA/anti-GBM testing, urgent biopsy and treatment without administrative delay."}
  ];
